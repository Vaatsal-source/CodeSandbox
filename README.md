...
How It Works
The user enters a coding task in the frontend.The frontend sends the prompt to the backend over WebSocket.The backend uses Gemini to generate initial Python code.The generated code is executed inside a Docker container.If execution fails, the debugger node analyzes the error and retries.the final code, explanation, and telemetry are saved to PostgreSQL.The frontend displays live updates, execution output, and history.
